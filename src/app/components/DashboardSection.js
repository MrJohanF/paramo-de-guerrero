...
const Card = ({ title, description, icon }) => {
  ...
  return (
    <motion.div
      ...
      className={`${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} rounded-lg shadow-md p-4`}
    >
      ...
      <button className={`text-sm text-green-600 ${isDarkMode ? 'bg-green-900' : 'bg-green-100'} px-3 py-1 rounded-md hover:bg-green-200 transition-colors`}>
        Ver detalles
      </button>
    </motion.div>
  );
...
const AnalyticsCard = ({ title, value, change, icon: Icon }) => {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{title}</span>
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setRevenueData(prevData => [
        ...prevData.slice(-9),
        { name: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}), value: Math.floor(Math.random() * 1600) }
      ]);
    }, 3000);
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={revenueData}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: isDarkMode ? '#1F2937' : 'white', border: `1px solid ${isDarkMode ? '#374151' : '#e5e7eb'}`, color: isDarkMode ? 'white' : 'black' }} />
